<template>
  ...
  <div class="dialog__browse-file" @click.stop="(()=>{})">
    <!-- 弹窗 -->
    <mt-popup v-model="show2" width="100%" height="100%" center :before-close="beforeClose" :modal-append-to-body="true"
      ...
      <div v-else style="text-align: center;">
        <iframe :src="reviewHref" width="82%" height="900"></iframe>
      </div>
    </mt-popup>
  </div>
</template>
...
<script>
  export default {
          ...
          if(vm.file.isGeneratedFile){
            ...
            if(vm.show&&vm.file&&materialPath){
              ...
              if(".jpg" == suffix || ".jpeg" == suffix || ".png" == suffix || ".JPG" == suffix || ".JPEG" == suffix || ".PNG" == suffix){
                ...
                vm.Toast("只能支持.pdf、.docx、.doc、.png、.jpg、.jpeg、.JPG、.JPEG、.PNG、.DOC、.DOCX、.PDF格式")
              }
    ...
    methods: {
      noSubmitPreview() {
        let vm = this;
        vm.requestPost('/preview/previewByContent', vm.formData).then(response => {
          if (response.data.status === "success") {
            let filePath = response.data.filePath;
            filePath = filePath.replace(/\\/g, '/');
            let domain, officeUrl;
            vm.requestPost('/preview/getContentPreviewConfig').then(function (response) {
              if (response.data.status === "success") {
      ...
      byFileStream() {
        let vm = this;
        let domain, officeUrl;
        vm.requestPost('/preview/getConfiguration').then(function (response) {
          if (response.data.status === "success") {
            domain = response.data.serverDomain;
            officeUrl = response.data.officeappsUrl;
      ...
      preview(path) {
        ...
        vm.requestPost('/preview/getConfiguration?filePath=' + path).then(function (response) {
          if (response.data.status === "success") {
            domain = response.data.serverDomain;
            officeUrl = response.data.officeappsUrl;
