  ...
  <div class="Issue">
    ...
    <div v-infinite-scroll="loadMore" infinite-scroll-distance="10">
      <ul class="contentlist" v-for="(item, index) in tabdata" :key="index">
        ...
        <li>
          <span>发文形式</span>
          <span>{{ item.sendType=='1'?'快递':item.sendType=='2'?'手递':item.sendType=='3'?'电邮':item.sendType=='4'?'传真':''}}</span>
        </li>
        <li>
          <span>收件单位</span>
          <span>{{ item.receiveUnit }}</span>
        </li>
        <li>
          <span>收件人</span>
          <span>{{ item.receivePeople}}</span>
        </li>
        <li>
          <span>文件形式</span>
          <span>{{ item.fileType=='1'?'纸版':item.fileType=='2'?'电子版':item.fileType=='3'?'纸版和电子版':'' }}</span>
        </li>
        <li>
          <span>份数</span>
          <span>{{ item.copyNum}}</span>
        </li>
        <li>
          <span>备注</span>
...
export default {
  ...
  methods: {
    init() {
      ...
      // 获取列表数据
      vm.requestPost('/wholeProcess/letter/query/searchLetterPeriodList', formData)
        .then(res => {
    ...
    download(item) {
      this.downloadFile(item.letterFilePath, item.letterFileName);
    },
  }
};
</script>
