<template>
  <!-- 工作台-已办-->
  <div class="work__table-handle" v-infinite-scroll="loadMore">
    ...
    <ul v-for="(item, i) in tableData" class="contentlist" :key="i">
      ...
      <li class="contentlist-li" v-if="dataVOParamKeyMap.tenderTaskSts">
        <span>状态</span>
        <span>{{ item.tenderTaskSts==1?'未中标':'已中标'}}</span>
      </li>
      ...
      <li class="contentlist-li" v-if="dataVOParamKeyMap.endProjectUserName">
        <span>项目结束人</span>
        <span>{{ item.endProjectUserName }}</span>
      </li>
      <li class="contentlist-li" v-if="dataVOParamKeyMap.endDate">
        <span>项目结束时间</span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        userPowerInfo:this.SStorage.getItem("userPowerInfo")// 登录用户数据
      };
    },
    mounted() {
      let vm = this;
    ...
    methods: {
      ...
      currentChangePage(currentPage, pageSize) { //分页方法
        ...
        vm.requestPost('/work/query/searchHandleList', formData).then(res => {
