<template>
  ...
  <div class="createTask">
    ...
    <form class="projectForm" id="projectForm" @submit.prevent="submitForm">
      ...
      <div class="form-item" @click="state && sel()" v-if="createList.departmentId === 'db5e76f9-9fbd-11e9-a036-0e411fd5cf91'">
        <mt-cell title="合同类型">
      ...
      <div class="form-item" @click="state && sel1()">
        <mt-cell title="咨询类别">
          <input
            type="text"
            readonly="readonly"
...
export default {
  ...
  data() {
    return {
      ...
      categorylist: [] /* 咨询类别数据 */,
      personlist: [] /*, 执行人数据 */,
      state: this.createState == 0 ? true : false
    };
  },
  components: {
  ...
  methods: {
    ...
    sel1() {
      this.popupVisibleCategory = true;
    },
    verify(val) {
      /* 自定义验证 */
      var state = true;
      ...
      vm.$validator
        ...
        .then(result => {
          if (result) {
            var currentTaskform = [
              {
                contractId: vm.createList.contractId /* 合同id*/,
                contractTypeId: vm.contractTypeId /* 合同类型id */,
                counselTypeId: vm.counselTypeId /* 咨询类别id */,
                taskName: vm.projectForm.taskName /* 任务名称 */,
                taskId: vm.projectForm.taskId,
                userList: [
                  /* 主提审神人 */
                  {
                    userId: vm.projectForm.authorizedId /* 编制成果文件id */,
                    taskContent: vm.projectForm.general /* 任务概况 */,
                    needHour: vm.projectForm.manHour /* 工时 */,
                    isMainDuty: '1',
                    endDate: vm.projectForm.Pickertext /* 完成时间 */
                  }
                ]
              }
            ];
            ...
            if (vm.createState == 0) {
              ...
              params = {
                // 设置合同id
                contractId: vm.createList.contractId,
                // 表单数据
                ...
                vm.requestPost(url, params)
                  .then(res => {
                    if (res.data.status == 'success') {
                      ...
                      vm.$emit('cancelTask');
                      // 任务添加或修改任务后修改目标节点,然后刷新页面
                      let storage = vm.SStorage.getItem('/app/foundItem');
                      if (params.contractId) {
                        // 创建任务
                        storage.businessId = res.data.primaryKey;
                      } else {
                        // 修改任务
                        storage.businessId = params.taskList[0].taskId;
                      }
