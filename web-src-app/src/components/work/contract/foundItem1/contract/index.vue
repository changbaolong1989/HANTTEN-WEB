...
<script>
  ...
  import right from './right.vue'; // 合同表单
  export default {
    components: {
      ...
      vm.$nextTick(function() {
        ...
        vm.bus.$on('changeform', (val,boo) => {
          vm.$refs['baseContract'].getData().then(v =>{
            // v:如果表单校验成功返回的是对象数据,否则返回的是Boolean值false
            if(v)vm.submit(val,boo,v);
          });
        });
      });
    },
        ...
        if (contractData.contractType == '0') {
          ...
          switch (contractData.formworkType) {
            case "0":
              ...
              if (retServiceAndWorkContentListData && retServiceAndWorkContentListData.length > 0) {
                ...
                for (let i = 0; i < retServiceAndWorkContentListData.length; i++) {
                  ...
                  serviceAndWorkContentItem.isChargeRadixProvisional = serviceAndWorkContentItemData.charge.state?'1':'0';
                  // 收费标准
                  serviceAndWorkContentItem.chargeStandard = serviceAndWorkContentItemData.standard.money;
                  // 收费标准是否暂定(是否暂定 0：未选中 1：选中)
                  serviceAndWorkContentItem.isChargeStandardProvisional = serviceAndWorkContentItemData.standard.state?'1':'0';
                  // 酬金金额
                  serviceAndWorkContentItem.remuneration = serviceAndWorkContentItemData.sum.money;
                  //  酬金金额是否暂定(是否暂定 0：未选中 1：选中)
                  serviceAndWorkContentItem.isRemunerationProvisional = serviceAndWorkContentItemData.sum.state?'1':'0';
                  // 备注
            ...
            case "2":
              ...
              formworkContent.isDesignPayProvisional = contractInfoData.isDesignPayProvisional?'1':'0'; // 设计阶段服务报酬暂定
              formworkContent.isConstructionPayProvisional = contractInfoData.isConstructionPayProvisional?'1':'0'; // 施工阶段服务报酬暂定
              formworkContent.isSettlementPayProvisional = contractInfoData.isSettlementPayProvisional?'1':'0'; // 结算阶段服务报酬暂定
              formworkContent.isAdvancePaymentProvisional = contractInfoData.isAdvancePaymentProvisional?'1':'0'; // 预付款暂定
              formworkContent.isProgressPaymentProvisional = contractInfoData.isProgressPaymentProvisional?'1':'0'; // 进度款暂定
              formworkContent.isBalanceProvisional = contractInfoData.isBalanceProvisional?'1':'0'; // 余款暂定
            ...
            case "3":
              ...
              formworkContent.isInstalCostProvisional = contractInfoData.isInstalCostProvisional?'1':'0'; //工程概算投资额或建筑安装工程费暂定
              ...
              formworkContent.isSignAmountProvisional = contractInfoData.isSignAmountProvisional?'1':'0'; //请输入签约酬金与补偿费用 暂定
...
//                    let tempAform = this.$refs['contract'].$refs['formCostConsulting'].$refs['serviceAndWorkContent']._data.Aform ;
                    ...
                    for (let i = 0; i < tempServiceAndWorkContentList.length; i++) {
                      ...
                      // 收费基数暂定
                      dataItem.charge.state = !!parseInt(tempServiceAndWorkContentList[i].isChargeRadixProvisional);
                      // 收费标准
                      dataItem.standard = {};
                      dataItem.standard.money = tempServiceAndWorkContentList[i].chargeStandard;
                      // 收费标准暂定
                      dataItem.standard.state = !!parseInt(tempServiceAndWorkContentList[i].isChargeStandardProvisional);
                      dataItem.sum = {};
                      // 酬金金额
                      dataItem.sum.money = tempServiceAndWorkContentList[i].remuneration;
                      // 酬金金额暂定
                      dataItem.sum.state = !!parseInt(tempServiceAndWorkContentList[i].isRemunerationProvisional);
                      // 备注
...
//                    let tempCform = this.$refs['contract'].$refs['formCostConsulting'].$refs['provideData']._data.Cform;
                ...
                case "2":
                  // 工程管理咨询服务合同
                   ...
                   // 暂定转换
                   // 设计阶段服务报酬暂定
                   vm.currentTransmitData.formProjectManagementConsultingService.form.isDesignPayProvisional =!!parseInt(vm.currentTransmitData.formProjectManagementConsultingService.form.isDesignPayProvisional);
                   // 施工阶段服务报酬暂定
                   vm.currentTransmitData.formProjectManagementConsultingService.form.isConstructionPayProvisional =!!parseInt(vm.currentTransmitData.formProjectManagementConsultingService.form.isConstructionPayProvisional);
                   // 结算阶段服务报酬暂定
                   vm.currentTransmitData.formProjectManagementConsultingService.form.isSettlementPayProvisional =!!parseInt(vm.currentTransmitData.formProjectManagementConsultingService.form.isSettlementPayProvisional);
                   // 预付款暂定
                   vm.currentTransmitData.formProjectManagementConsultingService.form.isAdvancePaymentProvisional =!!parseInt(vm.currentTransmitData.formProjectManagementConsultingService.form.isAdvancePaymentProvisional);
                   // 进度款暂定
                   vm.currentTransmitData.formProjectManagementConsultingService.form.isProgressPaymentProvisional =!!parseInt(vm.currentTransmitData.formProjectManagementConsultingService.form.isProgressPaymentProvisional);
                   // 余款暂定
                   vm.currentTransmitData.formProjectManagementConsultingService.form.isBalanceProvisional =!!parseInt(vm.currentTransmitData.formProjectManagementConsultingService.form.isBalanceProvisional);
                  break;
                case "3":
                  // 监理合同
                   ...
                   // 暂定转换
                   //工程概算投资额或建筑安装工程费暂定
                   tempFormSupervision.isInstalCostProvisional = !!parseInt(tempFormSupervision.isInstalCostProvisional);
                   //请输入签约酬金与补偿费用 暂定
                   tempFormSupervision.isSignAmountProvisional = !!parseInt(tempFormSupervision.isSignAmountProvisional);
                  // 监理期限
                    // 开始时间
