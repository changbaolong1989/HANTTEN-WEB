  ...
  <div class="foundItem__contract--right-default">
    <el-collapse v-model="activeNames" accordion style="margin: 0;">
      ...
      <collapse-up-and-download
        v-if="typeFlag == 7||archiveList.length>0"
        res="contractTemplate"
        name="download2"
        title="上传归档附件"
        :multiple="true" :limit="1"
        :showUploadBtn="typeFlag == 7"
        :showDelBtn="typeFlag == 7"
        :files="archiveList"
        @uploadFile="uploadFileArchive"
        @del="deleteFileInfo"
        :flush="true"></collapse-up-and-download>
    </el-collapse>
    ...
    <div class="btn-group primary">
      <p>
        <!-- <mt-switch v-model="above" v-if="<strong>jumpFlag</strong>&&(typeFlag==1||typeFlag==2||typeFlag==3||typeFlag==4)" @change="changeAbove">
          是否跳级审核
        </mt-switch> -->

        <mt-switch
          v-model="above"
          v-if="(jumpFlag&&(typeFlag==1||typeFlag==2||typeFlag==3||typeFlag==4)&&(currentTransmitData.data.departmentId == 'db5e76f9-9fbd-11e9-a036-0e411fd5cf91'|| currentTransmitData.data.departmentId == 'dea3cd5d-9fbd-11e9-a036-0e411fd5cf91'))
              ||(jumpFlag&&(typeFlag==1||typeFlag==2)&&(currentTransmitData.data.departmentId == 'eb1f8c91-960d-4717-84ee-cc3b8137ccd3'))"
          @change="changeAbove">
          是否跳级审核
        </mt-switch>
      </p>
      <button class="linear" @click="present(this)" v-if="typeFlag==1">提交审核</button>
      ...
      <button class="linear" v-if='typeFlag==1' @click="present(this,true)">预览</button>
    </div>
    <!-- 可写组件 - 对话框 -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteFileInfo(val) {
        let vm  = this;
          vm.requestPost('/contract/delete/deleteContractArchiveFileInfo', {
            ...
            if (res.data.status == "success") {
              ...
              vm.Toast('error', res.data.message)
            }
          }).catch(e => {
            vm.Toast('error', e)
          })
      ...
      submitArchiveButton() {
        ...
        vm.MessageBox.confirm('确定执行此操作?').then(action => {
          ...
          // 走提交归档接口
          vm.requestPost('/contract/modify/submitArchiveHTContract', formData).then(res => {
      ...
      sealButton() {
        ...
        vm.MessageBox.confirm('确定执行此操作?').then(action => {
          ...
          // 走盖章接口
          vm.requestPost('/contract/modify/confirmSealHTContract', formData).then(res => {
            vm.Toast(res.data.message);
            // if (res.data.status != 'failure') history.go(-1);
            if (res.data.status != 'failure') vm.$router.push('/app/workbench');
          }).catch(err => {
      ...
      reviewContract(isPass) {
        let vm = this;
        vm.MessageBox.confirm('确定执行此操作?').then(action => {
          vm.bus.$emit('reviewContract', isPass, vm.tool.deepCopy(vm.object, vm.tool.deepCopy));
        }).catch(err => {});;
      },
      /**
       * @method 撤回
      ...
      revokeHTContract() {
        ...
        vm.MessageBox.confirm('确定执行此操作?').then(action => {
          ...
          vm.requestPost('/contract/revoke/revokeHTContract', formData).then(res => {
            vm.Toast(res.data.message);
      ...
      present(e,boo) {
        let vm = this;
        vm.bus.$emit("changeform", vm._data,!!boo);
      }
    }
  }
</script>
