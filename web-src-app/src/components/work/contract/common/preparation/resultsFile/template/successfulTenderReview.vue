<template>
  <!-- 合同审核意见（招标）模板-->
  <div class="successfulTenderReview">
    <form class="projectForm">
      ...
      <div class="block">
        ...
        <div class="form-item">
          <label>合同</label>
          <input type="text" placeholder="请输入合同" v-model="templateform.input6"  v-validate="'required'" name="input6" data-vv-as="合同" />
        </div>
        <p>审核意见</p>
        <div class="form-item zanding">
          <label for="">合同金额<i class="iconfont iconhelp" @click.stop="help('金额单位为元，金额千分符，小数点后两位')"></i> </label>
          <div class="zandiv">
            <input type="text" placeholder="请输入合同金额" v-model="templateform.input7" v-validate="'required'" name="input7" data-vv-as="合同金额"  />
            <span>元</span>
          </div>
        </div>
      </div>
