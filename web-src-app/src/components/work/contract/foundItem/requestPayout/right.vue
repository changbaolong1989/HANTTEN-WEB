...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      preview() {
        let vm = this;
        vm.bus.$emit('requestPreview', this.above);
      },
      /**
       * @method 删除打款凭证文件
       */
      ...
      auditingHTQuestionMenu(isPass){
        ...
        this.requestPost('/requestMenu/modify/auditingQuestionMenu', paramObj).then(res => {
            Toast({
      ...
      commitBillStep() {
        ...
        this.requestPost('/requestMenu/modify/commitBillStep', formData).then(res => {
          if (res.data.status == 'failure') {
            Toast({
              message: res.data.message,
              position: 'middle',
      ...
      recallQuestionMenu(){
        ...
        this.requestPost('/requestMenu/modify/recallQuestionMenu', formData).then(res => {
          if (res.data.status == 'failure') {
            Toast({
              message: res.data.message,
              position: 'middle',
      ...
      confirmVoucherStep(){
        ...
        this.requestPost('/requestMenu/modify/confirmVoucherStep', formData).then(res => {
          if (res.data.status == 'failure') {
            Toast({
              message: res.data.message,
              position: 'middle',
