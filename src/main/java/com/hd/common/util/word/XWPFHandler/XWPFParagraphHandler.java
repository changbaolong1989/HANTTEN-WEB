...
public class XWPFParagraphHandler {
	 ...
	 * @return
	 */
	public boolean replaceAll(Map<String,Object> param){
		...
		for(XWPFRun run : allXWPFRuns){
			if(newRunContext.containsKey(run)){
				...
				if (newContext.contains("\n")){
					String[] text = newContext.split("\n");

					for (int i = 0;i<text.length;i++){

						if (i == 0){
							XWPFRunHandler.setText(run,text[i]);
						}else {

							XWPFRun tempRun = paragraph.insertNewRun(2 * i - 1);

							tempRun.setText(text[i]);
							// 设置字体  小四号
							tempRun.setFontSize(run.getFontSize());
							tempRun.setColor(run.getColor());
							tempRun.setFontFamily(run.getFontFamily());
							paragraph.insertNewRun(2 * i - 1).addCarriageReturn();
							runFlg = true;
						}
