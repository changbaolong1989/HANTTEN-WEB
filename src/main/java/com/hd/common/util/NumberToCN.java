...
//            "五", "六", "七", "八", "九" };
     ...
     * @return 对应的汉语大写
     */
    public static String numberToCNUpper(BigDecimal paramNumber) {
        StringBuffer sb = new StringBuffer();

        int signum = paramNumber.signum();
        // 零的情况
        if (signum == 0) {
            return CN_UPPER_NUMBER[0];
        }
        //这里会进行四舍五入
        long number = paramNumber.movePointRight(MONEY_PRECISION)
                .setScale(0, 4).abs().longValue();
        // 得到小数点后两位值
        long scale = number % 100;
        int numUnit = 0;
        ...
        while (true) {
            ...
            if (numUnit > 0) {
                ...
                if (!(getZero) && numIndex > 2) {
                 ...
                 if (((numIndex - 2) % 4 == 0) && (number % 1000 > 0) && numIndex > 2) {
                    sb.insert(0, CN_UPPER_MONETRAY_UNIT[numIndex]);
                }
                getZero = true;
            }
