  ...
  <div class="mh-set-role">
    ...
    <el-row type="flex" class="row-bg" justify="end">
      <el-col :span="24" class="text-align-right"><!-- 新增/修改-->
        <div class="item-tab-chuang" @click="addEstimateBtn()" style="float: right;">
    ...
    <div class="item-tab-bottom">
      <el-table :data="tabdata" border style="width: 100%">
        ...
        <el-table-column align="center" class-name="table-column" prop="estimateName" label="概算名称/版本" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <span>{{ !scope.row.estimateName ? '-' : scope.row.estimateName }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" class-name="table-column" prop="floorSpace" label="建筑面积（m²）" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <span>{{ !scope.row.floorSpace ? '-' : scope.row.floorSpace }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" class-name="table-column" prop="estimatedCost" label="概算费用（元）" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <span>{{!scope.row.estimatedCost ? '-' : new Number(scope.row.estimatedCost)}}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" class-name="table-column" prop="estimationExParte" label="概算单方（元/m²）" :show-overflow-tooltip="true">
          <template slot-scope="scope">
        ...
        <el-table-column align="center" class-name="table-column" label="操作" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            ...
            <el-button type="text" class="modify" @click="updateEstimateBtn(scope.row)">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <estimate-form v-if = "centerDialogVisible" ref="childForm"></estimate-form>
  </div>
</template>

<script>
  import EstimateForm from '@/components/wholeProcessCost/projectDetails/budgetaryEstimate/estimateForm' /* 招标管理表单  */
  export default {
    data() {
    ...
    methods: {
      ...
      addEstimateBtn() {
        ...
        vm.$nextTick(() => {
          ...
          vm.$refs.childForm.addEstimateFunc();
        })
      },
      // 导出列表数据
      exportBtn() {
        ...
        elemIF.src = '/wholeProcess/estimate/export/exportEstimateData?realFileName=' + "概算.xls" + "&projectId=" +  vm.projectId;
      ...
      //查询概算信息(修改概算信息先行方法)
      updateEstimateBtn(row){
        const vm = this
        console.log("~~~~~~~~~~~~~~")
        vm.centerDialogVisible = true;
        vm.$nextTick(() => {
          console.log("this.$refs.childForm",vm.$refs.childForm)
          vm.$refs.childForm.updateEstimateFunc(row);
        })
      },
      // 查询列表数据
      searchList(currentPage,pageSize) {
        ...
        // 获取列表数据
        vm.apiAddress('/wholeProcess/estimate/query/searchProjectEstimatePeriodList', formData).then(res => {
      ...
      //下载文件
      downLoadFile(row){
        this.downloadFile(row.estimateFilePath,row.estimateFileName)
      },
    }
  }
</script>
