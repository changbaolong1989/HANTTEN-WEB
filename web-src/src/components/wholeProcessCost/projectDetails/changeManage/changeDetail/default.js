...
function getFormRules(customValidate) {
  return {
    ...
    alterName: [{required: true, message: '请输入变更名称', trigger: 'blur'}, {max: 20, message: '最大长度不能超过20', trigger: 'blur'}],
    professionalName: [{required: true, message: '请选择专业名称', trigger: 'change'}],
    hanttenAmount: [{required: true, message: '请输入审核金额', trigger: 'blur'},{validator: customValidate.moneyCheck, trigger: 'blur'}],
    contentSummary: [{max: 20, message: '最大长度不能超过20', trigger: 'blur'}],
    declareAmount: [{validator: customValidate.moneyCheck, trigger: 'blur'}],
    auditAmount: [{validator: customValidate.moneyCheck, trigger: 'blur'}],
    ownerAmount: [{validator: customValidate.moneyCheck, trigger: 'blur'}],
    symbol: [{validator: customValidate.checkNumAlphaSymbol, trigger: 'blur'}],
