<template>
  ...
  <div class="m-work-contract-foundItem">
    <!-- 中间视图 -->
    <div class="center--work-contract" v-if="showView">
    ...
    <!-- 右侧视图 -->
    <div width="200px" class="right--work-contract" v-if="showView"><component v-bind:is="view_right" :transmittedData="transmittedData"></component></div>
    <!-- 修改项目 -->
...
export default {
  ...
  methods: {
    ...
    baseContract(val) {
      ...
      if (val.treeNodeLevel == 1) {
        ...
        if (val.objType == 'requestMent') {
          ...
          // // 显示任务right
          this.view_right = 'task-r';

          // 显示项目编号等信息
          Object.assign(this.transmittedData, {
            showProjectNumFlg: true // 显示项目编号等信息
          });

          // 根据任务id查询合同内容
          this.apiAddress('/task/query/searchTaskContent', {
            // 任务id
            taskId: val.id
          })
            .then(res => {
              ...
              if (res.data.status == 'success') {
                ...
                if (res.data.dataVO.taskContent != '') {
                  // 显示任务说明
                  vm.transmittedData.taskContent = true;
                  // 设置任务说明
                  vm.transmittedData.taskDesc = res.data.dataVO.taskContent;
                } else {
                  // 隐藏任务说明
                  vm.transmittedData.taskContent = false;
                }
              } else {
                this.stateDialog('error', res.data.message);
              }
        ...
        console.log('pageContent', val.pageContent);

        console.log('periodType', val.periodType);

        console.log('relationId', val.relationId);

        console.log('departmentId', val.departmentId);

        console.log('taskId', val.taskId);

        // 隐藏项目编号等信息
        Object.assign(this.transmittedData, {
          // 显示步骤说明
          showPeriodDescFlg: true,
          // 隐藏任务说明
          taskContent: false,
          // 隐藏项目编号等信息
          showProjectNumFlg: false
        });
      }
    },
    // 获取部门缩写,并且设置数据
    ...
    delRequestMenu() {
      ...
      this.$confirm('此操作将作删除当前请款单信息, 是否继续?', '提示', {
