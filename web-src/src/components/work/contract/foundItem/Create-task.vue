<template>
  <!-- 创建任务弹窗 -->
    <el-dialog class="create-task" :title="title" :visible.sync="show" width="710px" transition="fade-in-linear" center @close="handleClose" :close-on-click-modal="false">
      <el-row>
        ...
        <el-form :model="item" ref="currentTaskform" :rules="rules">
          ...
          <el-row class="showTable" v-show="item.showTable">
            <el-table :data="item.tableData" border style="width: 100%">
              <el-table-column align="center" prop="taskName" label="已分配任务" :show-overflow-tooltip='true'></el-table-column>
              <el-table-column align="center" prop="endDate" label="完成时间" :show-overflow-tooltip='true'></el-table-column>
              <el-table-column align="center" prop="needHour" label="分配工时" :show-overflow-tooltip='true'></el-table-column>
            </el-table>
          </el-row>
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item label="完成时间" prop="endDate">
                <el-date-picker v-model="item.endDate" value-format="yyyy-MM-dd" type="date" placeholder="请输入完成时间" @change="isMainUserEndDate"></el-date-picker>
              </el-form-item>
            </el-col>
            <el-col :span="12">
  ...
  export default {
    data() {
      return {
        ...
        projectNum: "",
        // 大表单
        currentTaskform: this.taskform?[this.taskform]:[{
          // 任务id
          taskId: "",
          // 任务名称
          taskName: "",
    ...
    watch:{
        taskform:{
          handler(){
            let vm = this;
            vm.currentTaskform =  vm.tool.deepCopy([...[vm.taskform]], vm.tool.deepCopy);;
          },
         immediate: true
        }
    },
    ...
    methods: {
      ...
      handleClose() {
        ...
        console.log("currentTaskform",this.currentTaskform)
        // // 关闭弹窗
        vm.show = false;
      },
      // 关闭小表单
