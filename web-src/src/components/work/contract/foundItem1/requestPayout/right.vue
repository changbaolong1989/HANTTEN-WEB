  ...
  <div class="foundItem__requestPayout">
    <div class="given-container">
      <el-collapse class="level1" v-model="activeNames">
        ...
        <fileList
          title="开具发票"
          name="开具发票"
          v-if="showInvoiceCompFlag || requestInvoiceFileList.length > 0"
          :showUploadBtn="invoiceUploadBtnFlag"
          :delBtnFlag="invoiceDelBtnFlag"
          :uploadOptions="uploadOptions"
          @del="deleteFileInfo"
          :list="requestInvoiceFileList"
          :userType="2"
          @submit="issueInvoice"
        ></fileList>
        <fileList
          title="上传发票"
          name="上传发票"
          v-if="showUploadInvoiceCompFlag || invoiceFileList.length > 0"
          :showUploadBtn="uploadInvoiceUploadBtnFlag"
          :delBtnFlag="invoiceDelBtnFlag"
          :uploadOptions="uploadOptions"
          @del="deleteFileInfo"
        ...
        <fileList
          ...
          @del="deleteFileInfo"
          :list="paymentVoucherFileList"
          :userType="2"
          :isPayMoney="true"
          @submit="submitPaymentVoucherForm"
        ></fileList>
    ...
    <div class="btn-group">
      <p style="background-color: #B6C3CE;padding:0 20px;width: 100%;margin-bottom: 10px;">
        <el-checkbox v-model="above" style="color: white;" v-if="showJumpBtnFlag">跳级审核</el-checkbox>
      </p>

      <el-button type="primary" @click="commitBillStep" v-if="showInvoiceCompBtnFlag">开具发票</el-button>
      <el-button type="primary" @click="uploadBillStep" v-if="showUploadInvoiceCompBtnFlag">上传发票</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      uploadInvoiceUploadBtnFlag: false,
      ...
      showUploadInvoiceCompFlag: false, //上传发票
      showUploadInvoiceCompBtnFlag: false,
  ...
  methods: {
    ...
    auditingHTQuestionMenu(isPass) {
      ...
      this.$confirm(message, '提示', {
        ...
        .then(() => {
          ...
          let vm = this;
          this.apiAddress('/requestMenu/modify/auditingQuestionMenu', paramObj)
            .then(res => {
    ...
    submit() {
      this.$confirm('此操作将提交审核, 是否继续?', '提示', {
        ...
        .then(() => {
          this.bus.$emit('updateQuestionMenuInfo', this.above);
        })
        .catch(() => {});
    },
    commitBillStep() {
      this.$confirm('此操作将提交发票, 是否继续?', '提示', {
        ...
        .then(() => {
          let vm = this;
          if (!(vm.requestInvoiceFileList && vm.requestInvoiceFileList.length > 0)) {
            vm.$message({
              message: '请上传发票文件',
              type: 'error',
              center: true
            ...
            vm.apiAddress('/requestMenu/query/searchRequestMenuInfo', formData).then(res => {
              if (res.data.status == 'failure') {
                ...
                if (requestMenuInfo.requestNoteFileName==undefined||requestMenuInfo.requestNoteFileName=="") {
                  vm.$message({
                    message: '请等待提审人上传发票申请单附件后再操作',
                    type: 'error',
                    center: true
                  });
                  return;
                  ...
                  this.apiAddress('/requestMenu/modify/commitBillStep', formData)
                    .then(res => {
                      if (res.data.status == 'failure') {
                        vm.$message({
                          message: res.data.message,
    ...
    confirmVoucherStep() {
      this.$confirm('此操作将确认到款, 是否继续?', '提示', {
        ...
        .then(() => {
          ...
          this.apiAddress('/requestMenu/modify/confirmVoucherStep', formData)
            .then(res => {
              if (res.data.status == 'failure') {
                vm.$message({
                  message: res.data.message,
    ...
    recallQuestionMenu() {
      ...
      this.$confirm('是否撤回?', '提示', {
        ...
        .then(() => {
          this.apiAddress('/requestMenu/modify/recallQuestionMenu', formData)
            .then(res => {
              if (res.data.status == 'failure') {
                vm.$message({
                  message: res.data.message,
    ...
    issueInvoice(obj) {
      ...
      obj.delete('money');
      let vm = this;
      this.apiAddress('/requestMenu/modify/uploadInvoiceFile', obj)
        .then(res => {
          if (res.data.status == 'failure') {
            vm.$message({
              message: res.data.message,
    ...
    submitInvoiceForm(obj) {
      ...
      obj.delete('money');
      let vm = this;
      this.apiAddress('/requestMenu/modify/uploadBill', obj)
        .then(res => {
          if (res.data.status == 'failure') {
            vm.$message({
              message: res.data.message,
      ...
      vm.apiAddress('/requestMenu/query/searchRequestMenuInfo', formData)
        .then(res => {
          if (res.data.status == 'failure') {
            ...
            if (null != requestMenuInfo.requestInvoiceName && null != requestMenuInfo.requestInvoicePath) {
              let requestInvoiceFileObj = {
                materialName: requestMenuInfo.requestInvoiceName,
                materialPath: requestMenuInfo.requestInvoicePath,
                userName: '金额(元)' + requestMenuInfo.invoiceAmount
              };
              vm.$set(vm.requestInvoiceFileList, 0, requestInvoiceFileObj);
            }
    ...
    submitPaymentVoucherForm(obj) {
      ...
      obj.delete('payMoney');
      let vm = this;
      this.apiAddress('/requestMenu/modify/uploadPaymentVoucher', obj)
        .then(res => {
          if (res.data.status == 'failure') {
            vm.$message({
              message: res.data.message,
    ...
    showBtnFunc() {
      ...
      vm.uploadInvoiceUploadBtnFlag = false;
      ...
      vm.showUploadInvoiceCompFlag = false; //上传发票
      vm.showUploadInvoiceCompBtnFlag = false; //上传发票
      ...
      switch (userType) {
        ...
        case '5':
          ...
          break;
        case '56':
          vm.showUploadInvoiceCompFlag = true;
          vm.showUploadInvoiceCompBtnFlag = true;
          vm.uploadInvoiceUploadBtnFlag = true; //发票上传
          vm.invoiceDelBtnFlag = true; //发票删除
          break;
        case '6':
          vm.showInvoiceCompFlag = true;
    ...
    searchHTRequestMenuHisTaskInst(requestMenuId) {
      ...
      //查询审核意见列表
      vm.apiAddress('/requestMenu/query/searchHTRequestMenuHisTaskInst', formData)
        .then(res => {
          if (res.data.status == 'failure') {
            vm.$message({
              message: res.data.message,
