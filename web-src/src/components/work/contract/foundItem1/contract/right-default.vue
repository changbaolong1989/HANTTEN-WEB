  ...
  <div class="right-default">
    <div class="given-container">
      <el-collapse v-model="activeNames" accordion style="margin: 0;">
        <fileList res="contractTemplate" title="合同" :showUploadBtn="false" :list="contractList" :userType='userType' v-if="rowInfoLevel==3&&loadChild&&contractList.length>0"  type="contract" :id="transmittedData.id"></fileList>
        <fileList res="auditOpinion" title="审核意见文件" :showUploadBtn="false" :list="auditOpinionList" :userType='userType'
                  v-if="rowInfoLevel==3&&loadChild&&typeFlag==1&&auditOpinionList.length>0"></fileList>
        ...
        <!-- 面板-复核可写-->
        <collapse-review-write name="write1" :object="object" @submit="changeObject" v-if='typeFlag==2||typeFlag==3||typeFlag==4||typeFlag==5' :errorTyleList="errorTyleList"></collapse-review-write>
      </el-collapse>
      <!-- 提交审核弹窗组件 -->
      <dialog-submit-audit  :above="above" @cancel="cancel" :reviewer="reviewer" ></dialog-submit-audit>
      <el-collapse>
        <!-- 面板-上传下载 -->
        <el-collapse-item class="invoice" title="上传归档附件" v-if="list.length > 0 || typeFlag == 7">
          <template slot="title" v-if="typeFlag == 7">
            上传归档附件&nbsp;
            ...
            <div class="file-list">
              <ul>
                ...
                <li tabindex="0" class="el-upload-list__item is-finished" v-for="(f, index) in list" v-bind:key="index">
                  <a class="el-upload-list__item-name" style="width: 100%; margin-right: 0;" :title="f.materialName" @click="browseFile(f)">
                  ...
                  <i class="el-icon-close" @click.stop="deleteFileInfo(f)" v-if="typeFlag == 7"></i>
                </li>
    ...
    <div class="btn-group">
      <p style="background-color: #B6C3CE;padding:0 20px;width: 100%;margin-bottom: 10px;">
        <el-checkbox v-model="above"  style="color: white;" v-if='(this.jumpFlag&&(typeFlag==1||typeFlag==2||typeFlag==3||typeFlag==4)&&(this.currentTransmittedData.departmentId == "db5e76f9-9fbd-11e9-a036-0e411fd5cf91"|| this.currentTransmittedData.departmentId == "dea3cd5d-9fbd-11e9-a036-0e411fd5cf91"))||
        (this.jumpFlag&&(typeFlag==1||typeFlag==2)&&(this.currentTransmittedData.departmentId == "eb1f8c91-960d-4717-84ee-cc3b8137ccd3"))'>
          跳级审核
        </el-checkbox>
      </p>
      <el-input type="text" v-model="form.fileNumber" v-if='typeFlag==7' placeholder="请输入档案编号"></el-input>
      <el-button type="primary" v-if='typeFlag==7' @click="submitArchiveButton" style="margin-top: 10px;">提交归档</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      httpRequest(request) { // 自定义上传请求
        let vm = this,
            form = vm.fileFormData;
        if(vm.timeIndex){
          clearTimeout(vm.timeIndex);
          form.append('file', request.file);
          vm.timeIndex = vm.delayUpload(form);// 延时上传文件
        }else{
          form.append('file', request.file);
      ...
      auditingHTContractTask(isPass) {
        ...
        vm.$confirm('是否提交?', '提示', {
          ...
          vm.apiAddress('/contract/audit/auditingHTContract', paramObj).then(res => {
            if (res.data.status == 'failure') {
              vm.$message({
                message: res.data.message,
                type: 'error',
      ...
      revokeHTContract(){
        ...
        vm.$confirm('是否提交?', '提示', {
          ...
          vm.apiAddress('/contract/revoke/revokeHTContract', formData).then(res => {
            if (res.data.status == 'failure') {
              vm.$message({
                message: res.data.message,
                type: 'error',
