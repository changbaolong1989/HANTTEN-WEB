	...
	<div class="set-notice">
		...
		<el-row type="flex" class="row-bg item-open" justify="end">
			...
			<el-dialog title="新增" :visible.sync="centerDialogVisible" width="500px" transition="fade-in-linear" center>
				<el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
					<el-form-item label="技术标准名称" prop="standardName">
						<el-input placeholder="请输入技术标准名称" v-model="ruleForm.standardName"></el-input>
					</el-form-item>
					<el-form-item label="技术标准文件" class="upload upload1" prop="signPathFile">
						...
						<el-upload class="avatar-uploader" ref="upload" action="#"
						 ...
						 :data="ruleForm" :on-success="fileSuccess">
							...
							<br>
							<el-input placeholder="请输入技术标准文件名称" v-model="ruleForm.materialName" :disabled="true"></el-input>
						</el-upload>
					</el-form-item>
				</el-form>
			...
			<el-dialog title="修改" :visible.sync="centerDialogVisible1" width="500px" transition="fade-in-linear" center>
				<el-form :model="ruleForm1" :rules="rules" ref="ruleForm1" label-width="100px" class="demo-ruleForm">
					<el-form-item label="技术标准名称" prop="standardName">
						<el-input placeholder="请输入技术标准名称" v-model="ruleForm1.standardName"></el-input>
					</el-form-item>
					<el-form-item label="技术标准文件" class="upload upload1">
						<el-input v-model="ruleForm1.signPathFile" v-if="false"></el-input>
						<el-upload class="avatar-uploader" ref="upload" action="#"
						 :show-file-list="false" :on-change="handleAvatarSuccess1" :auto-upload="false" :before-upload="beforeAvatarUpload1"
						 :data="ruleForm" :on-success="fileSuccess">
							<el-button><span class="iconfont">&#xe614;</span>上传附件</el-button>
							<br>
							<el-input placeholder="请输入技术标准文件名称" v-model="ruleForm1.materialName" :disabled="true"></el-input>
						</el-upload>
					</el-form-item>
				</el-form>
				<span slot="footer" class="dialog-footer">
		...
		<div class="item-tab-bottom">
			...
			<el-table :data="tabdata" border style="width: 100%" class="item-tab">
				...
				<el-table-column align="center" class-name="table-column" label="技术标准" :show-overflow-tooltip="true" :resizable="false">
	...
	export default {
		...
		methods: {
			...
			refer() {
				const vm = this;
				// 发送查询列表数据请求
				vm.$axios.post('/config/technicalStandard/query/searchTechnicalStandardList', JSON.stringify({
						// 页码
						"pageNumber": 1,
						// 每页总条数
			...
			*/
			...
			submitForm(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						// 设置技术标准名称
						vm.formData.append("standardName ", vm.ruleForm.standardName);
						// 二次确认
						vm.$confirm('是否提交?', '提示', {
							...
							// 发送添加技术标准请求
							vm.$axios.post('/config/technicalStandard/add/saveTechnicalStandardInfo', vm.formData, {
								headers: {
			...
			submitForm1(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						// 设置技术标准id
						vm.formData1.append("standardId ", vm.standardId);
						// 设置技术标准名称
						vm.formData1.append("standardName ", vm.ruleForm1.standardName);
						// 二次确认
						this.$confirm('是否提交?', '提示', {
							...
							// 发送修改请求
							vm.$axios.post('/config/technicalStandard/modify/updateTechnicalStandardInfo', vm.formData1, {
								headers: {
			...
			changeTable(a, b) {
				...
				// 存储技术标准id,供修改时传入参数使用
				vm.standardId = b.standardId;
				// 发送查询详情请求
				vm.$axios.post('/config/technicalStandard/query/searchTechnicalStandardInfo', JSON.stringify({
						"standardId": b.standardId
					}), vm.config).
				then(function(response) {
					// 技术标准名称
					vm.ruleForm1.standardName = response.data.dataVO.standardName;
					// 技术标准名称
			...
			deleteTable(a, b) {
				...
				vm.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
					...
					// 发送删除请求
					vm.$axios.post('/config/technicalStandard/delete/deleteTechnicalStandardInfo',  JSON.stringify({
						"standardId": b.standardId
					}), {
						headers: {
							'Content-Type': 'application/json',
